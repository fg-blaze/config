...
function add_reviewers () {
    mapfile -t reviewers < ~/reviewers.txt;
    join_arr reviewers_str ' -a ' "${reviewers[@]:1}";
    host="${reviewers[0]}";
    CHANGE_ID="$1";
    LAST_CHANGE_ID="$(git log -1 | sed -rn 's/.*Change-Id:\s*(\S+)\b.*/\1/p')";
...
function check_pep8 () {
    ...
    if [[ ${#ERRORS[@]} -eq 0 ]]; then
        ...
        for f in ${ERRORS[@]}; do
            echo -e "\e[31m___________________________\e[0m"
            flake8 $f;
            echo -e "\e[31m___________________________\e[0m"
        done
        return 1
...
function git_review () {
    ...
    res=$?
    if [[ ${res} -eq 0 ]]; then
       git add -u && git commit --amend && git review    
    fi
}
